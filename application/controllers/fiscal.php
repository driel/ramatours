...
class Fiscal extends CI_Controller {
	...
	public function index($offset = 0) {
	    ...
	    if(strlen($q)){
	      	if(strlen($by)){
	        	$data["fiscal"] = $this->fiscal->list_where($q);
	      	}else{
	        	$data["fiscal"] = $this->fiscal->get_all($order_by, strtoupper($order), $this->perpage, $this->offset);
	      	}
	    }else{
	      	$data["fiscal"] = $this->fiscal->get_all("kurs_date", "ASC", $this->perpage, $this->offset);
	...
	private function _editData($id) {
	    ...
	    $data["form_action"] = "fiscal/update";

        $data['title'] = 'Edit Fiscal Kurs';
        $data['form_action'] = site_url('fiscal/save');
        $data['link_back'] = anchor('fiscal/', 'Back', array('class' => 'btn btn-danger'));
